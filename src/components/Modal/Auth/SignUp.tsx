...
const SignUp:React.FC = () => {
    ...
    const onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        createUserWithEmailAndPassword(singUpForm.email, singUpForm.confirmPassword);
    };
    ...
    return(
        <form onSubmit={onSubmit}>
            ...
            <Input required name='confirmPassword' placeholder='password' type='password' mb={2} onChange={ onChange} fontSize='10pt' _placeholder={{color: 'gray.500'}} _hover={{bg: 'white', border: ' 1px solid', borderColor: 'blue.500'}} _focus={{outline: "none", bg: 'white', border: ' 1px solid', borderColor: 'blue.500' }} bg="gray.50"  />
            <Text textAlign='center' color="" fontSize="10pt ">Password should be at least 6 characters</Text>
            <Text textAlign="center" mt={2} fontSize="10pt" color="red">{error || FIREBASE_ERRORS[userError?.message as keyof typeof FIREBASE_ERRORS]}</Text>
            <Button width='100%' height='36px' mt={2} mb={2} type='submit' isLoading={loading}>Sign Up </Button>
            <Flex fontSize="9pt" justifyContent="center">
                <Text mr={1}>Already a IIPEConnectian</Text>
